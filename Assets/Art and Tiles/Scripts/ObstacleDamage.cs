...
public class ObstacleDamage : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision) // This method is called when the GameObject this script is attached to collides with another GameObject
    {
        if (collision.gameObject.tag == "Player") // Check if the colliding GameObject has the tag "Player"
        {
            collision.gameObject.GetComponent<PlayerRespawn>().RespanwNew();
